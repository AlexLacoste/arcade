...
#define isOptions(disp, opt) \
    ((disp)->availableOptions() & arcade::displayer::IDisplay::AvailableOptions::opt)
            ...
            virtual std::unique_ptr<ISprite> createSprite(const std::string &spritePath,
                ...
                arcade::data::Vector2f scale = arcade::data::Vector2f{1, 1}) = 0;
    ...
    {
        if (a.left <= x && x <= (a.left + a.width) && a.top <= y && y <= (a.top + a.height)) {
            return true;
        }
        return false;
    };
    ...
    {
        if (a.left <= b.x && b.x <= (a.left + a.width) && a.top <= b.y
            && b.y <= (a.top + a.height)) {
            return true;
        }
        return false;
    ...
    {
        if ((((a.left <= b.left && b.left <= (a.left + a.width))
